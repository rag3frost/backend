...
def predict_cancer():
    try:
        ...
        # Load encoders
        encoders = joblib.load('models/cancer_encoders.pkl')
        
        # Apply encoding to categorical variables
        input_data['Gender'] = encoders['gender'].transform([data['Gender']])
        input_data['Smoking'] = encoders['smoking'].transform([data['Smoking']])
        input_data['GeneticRisk'] = encoders['genetic_risk'].transform([data['GeneticRisk']])
        input_data['PhysicalActivity'] = encoders['activity'].transform([data['PhysicalActivity']])
        input_data['AlcoholIntake'] = encoders['alcohol'].transform([data['AlcoholIntake']])
        input_data['CancerHistory'] = encoders['cancer_history'].transform([data['CancerHistory']])
